...
function tablaRespuestaRes(items) {
    console.log(items[1]);
    let myTableRes = `<table BORDER CELLPADDING=2 BORDERCOLOR='#193D61'><th scope='col'> ID RESERVATION </th><th>START DATE</th><th>DEVOLUTION DATE</th><th>STATUS</th><th>BOAT</th><th>ID CLIENT</th><th>FULL NAME</th><th>EMAIL</th><th>SCORE</th>`;
    for (let i = 0; i < items.length; i++) {
        ...
        myTableRes += `<td> <button class="btn-select" value="${items[i].idReservation}" style="background-color:#364B61; border-color:#193D61; color:white;">Change</button></td>`;
        myTableRes += `<td> <button class="btn-delete" value="${items[i].idReservation}" style="background-color:#364B61; border-color:#193D61; color:white;">Delete</button>`;
        myTableRes += `</tr>`;
    }
