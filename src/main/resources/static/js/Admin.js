...
function tablaRespuestaAdmin(items) {
    let myTableAdmin = `<table BORDER CELLPADDING=2 BORDERCOLOR='#193C61'> <th scope='col'>  FULL NAME </th><th> EMAIL </th>`;
    for (let i = 0; i < items.length; i++) {
        myTableAdmin += `<tr>`;
        myTableAdmin += `<td>${items[i].name}</td>`;
        myTableAdmin += `<td>${items[i].email}</td>`;
        myTableAdmin += `<td> <button class="btn-select" value="${items[i].id}" style="background-color:#364B61; border-color:#193C61; color:white;" >Change</button></td>`;
        myTableAdmin += `<td> <button class="btn-delete" value="${items[i].id}" style="background-color:#364B61; border-color:#193C61; color:white;" >Delete</button></td>`;
        myTableAdmin += `</tr>`;
    }
