...
function tablaRespuestaMes(items) {
    console.log(items[0])
    let myTableMes = `<table BORDER CELLPADDING=2 BORDERCOLOR='#193C61'><th scope='col'> MESSAGE </th><th>BOAT</th><th>CLIENT</th>`;
    for (let i = 0; i < items.length; i++) {
        ...
        myTableMes += `<td> <button class="btn-select" value="${items[i].idMessage}" style="background-color:#364B61; border-color:#193C61; color:white;">Change</button></td>`;
        myTableMes += `<td> <button class="btn-delete" value="${items[i].idMessage}" style="background-color:#364B61; border-color:#193C61; color:white;">Delete</button>`;
        myTableMes += `</tr>`;
    }
