...
function mostrarInformacionBot() {
    $.ajax({
        url: host + '/Boat/all',
        type: 'GET',
        dataType: "JSON",
...
function tableRespuestaBot(items) {
    let myTableBot = `<table BORDER CELLPADDING=2 BORDERCOLOR='#193C61'><th scope='col'> FULL NAME </th><th> DEPARTMENT </th><th> YEAR </th><th> DESCRIPTION </th><th> CATEGORY</th>`;
    for (let i = 0; i < items.length; i++) {
        ...
        myTableBot += `<td> <button class="btn-select" value="${items[i].id}" style="background-color:#364B61; border-color:#193C61; color:white;">Change</button></td>`;
        myTableBot += `<td> <button class="btn-delete" value="${items[i].id}" style="background-color:#364B61; border-color:#193C61; color:white;">Delete</button></td>`;
        myTableBot += `</tr>`;
    }
    $("#resultadoBot").append(myTableBot);
    myTableBot = `</table>`;
    listeners();
}
function listeners(){
