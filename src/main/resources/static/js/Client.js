...
function tablaRespuestaCli(items) {
    let myTableCli = `<table BORDER CELLPADDING=2 BORDERCOLOR='#193C61'><th scope='col'> EMAIL </th><th> FULL NAME </th><th> AGE </th>`;
    for (let i = 0; i < items.length; i++) {
        myTableCli += `<tr>`;
        myTableCli += `<td>${items[i].email}</td>`;
        myTableCli += `<td>${items[i].name}</td>`;
        myTableCli += `<td>${items[i].age}</td>`;
        myTableCli += `<td> <button class="btn-select" value="${items[i].idClient}" style="background-color:#364B61; border-color:#193C61; color:white;">Change</button></td>`;
        myTableCli += `<td> <button class="btn-delete" value="${items[i].idClient}" style="background-color:#364B61; border-color:#193C61; color:white;">Delete</button></td>`;
        myTableCli += `</tr>`;
